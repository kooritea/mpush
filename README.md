...
```javascript
{
  ...
  "fcm": {
    "comfirmMode": false, // 确认模式,当打开时,如果没有收到客户端的确认信息将会一直重发同一条消息直至客户端确认，一般不用打开，如果你的的客户端设备不支持fcm拉起应用可以尝试打开（mpush客户端支持确认模式的前提下），如果使用了类似绿色守护捐赠版xposed特性的为绿色化应用开启GCM推送则完全不需要打开这个选项。
